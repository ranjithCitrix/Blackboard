...
function main(args: OptionValues) {
  ...
  console.warn(
    "*** Warning: building script is not not part of console runner execution yet, `npm run build` !!! ***"
  );
  ...
  const consoleRunnerJar =
    "console-runner-sdk/scripting-service-console-runner.jar";
  ...
  console.log(
    `console runner arguments: "${consoleRunnerArguments.join(" ")}"`
  );

  const consoleRunner = spawn("java", [
    "-cp",
    consoleRunnerJar,
    "com.citrix.cdi.script.runner.cmdparameters.RunCommand",
    ...consoleRunnerArguments,
  ]);

  consoleRunner.stdout.on("data", (data) =>
    console.log(`stdout: ${bufferToConsole(data)}`)
  );
  consoleRunner.stderr.on("data", (data) =>
    console.log(`stderr: ${bufferToConsole(data)}`)
  );
...
function expandParameters(parameters: Parameters): string[] {
  return Object.entries(parameters).reduce((acc: string[], [name, value]) => {
