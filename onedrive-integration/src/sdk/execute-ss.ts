...
import {encodeRequestBody, formatResult} from "../../../commons/src/lib/integrationServicePlugins";
    ...
    return build()
        .then(() => {
            ...
            const executeRequestBody: IScriptExecRequest = {
                ...
                functionName: args.functionName || scriptExecRequestTemplate.functionName,
                parameters: parameters,
            }
        ...
        .then(getSuccessJson(async response => {
            return new Error(`Request to /api/script/execute failed: ${await response.text()}`)
        }))
