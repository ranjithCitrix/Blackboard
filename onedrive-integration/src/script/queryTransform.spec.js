...
import {transformArrayExpression, transformQuery} from './queryTransform'
...
describe('Utility functions', () => {
    describe('transformArrayExpression', () => {
        it('no item', () => {
            expect(transformArrayExpression('AND', [])).to.be.equal('')
        })
        it('single item', () => {
            expect(transformArrayExpression('AND', [ftcontains('hello')])).to.be.equal('hello')
        })
        it('multiple single items', () => {
            expect(transformArrayExpression('AND', [ftcontains('hello'), ftcontains('world')])).to.be.equal('(hello AND world)')
        })
    })
})